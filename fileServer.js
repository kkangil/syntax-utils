...
const fileStorage = multerS3({
  ...
  key: (req, file, cb) => {
    ...
    cb(null, `easeNmore/${moment().format('YYYY')}/${moment().format('MM')}/${moment().format('DD')}/${moment().format('HHmm')}/${files[0]}.${files[files.length - 1]}`)
  }
})
  ...
  if (req.files.length > 0) {
    ...
    Jimp.read(req.files[0].buffer, function (err, image) {
      ...
      .then(function (result) {
        ...
        s3Params.original = {
          Bucket: config.aws.s3.bucket,
          Key: 'easeNmore/' + yyyy + '/' + mm + '/' + dd + '/' + ss + '_' + req.files[0].originalname,
          ACL: 'public-read',
        ...
        s3Params.thumbnail = {
          Bucket: config.aws.s3.bucket,
          Key: 'easeNmore/' + yyyy + '/' + mm + '/' + dd + '/thumbnail_' + ss + '_' + req.files[0].originalname,
          ACL: 'public-read',
          ContentType: 'image/' + extra
        }
